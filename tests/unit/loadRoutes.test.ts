...
describe("loadRoutes", () => {
    ...
    test("registers method+path from Next.js-style files (happy path)", async () => {
        ...
        await writeFile(join(tempDir, "users.ts"), `export const GET = async () => new Response("users");\n`);
    ...
    test("propagates import failures", async () => {
        ...
        await writeFile(join(tempDir, "broken.ts"), `throw new Error("intentional import failure");\n`);

        const router = new Router();
        await expect(loadRoutes(tempDir, router)).rejects.toThrow("intentional import failure");
    });
    ...
    test("throws on method+path collision with existing routes", async () => {
        ...
        await writeFile(join(tempDir, "users.ts"), `export const GET = async () => new Response("file");\n`);

        const router = new Router();
        router.get("/users", createMockHandler({ source: "fluent" }));

        await expect(loadRoutes(tempDir, router)).rejects.toThrow("Route collision: GET /users");
    });
});
